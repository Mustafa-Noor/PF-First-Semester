...
int employeemenu();
int customermenu();
int printMitems(string arrM[], int priceM[], int availableM[], int quantityM[], int &menq, int cardindex, int quantforMen[][30]);
int printWitems(string arrW[], int priceW[], int availableW[],int quantityW[], int &womenq, int cardindex, int quantforWomen[][30]);
int bill(string arrM[], string arrW[], int totalM[], int totalW[], int priceM[], int priceW[], int quantityM[], int quantityW[], int menq, int womenq, int cardindex, int finalTotal[], string delivery[], string userArea[], bool deliveryop, bool delArea, int quantforMen[][30], int quantforWomen[][30]);
int PayBill(int totalM[], int totalW[], int quantityM[], int quantityW[], int priceM[], int priceW[], int availableM[], int availableW[], int menq, int womenq, bool &delArea, bool &deliveryop, bool &billpaid, int cardindex, int quantforMen[][30], int quantforWomen[][30], string delivery[], string userArea[]);
int cart(string arrM[], string arrW[], int quantityM[], int quantityW[], int menq, int womenq, int cardindex, int quantforMen[][30], int quantForWomen[][30]);
int status(bool billpaid);
int updatequantity(int quantityM[], int quantityW[], string arrM[], string arrW[], int menq, int womenq, int availableM[], int availableW[], int counter, int cardindex, int quantforMen[][30], int quantforWomen[][30]);
...
void resetCart(int menq, int womenq, int quantityM[], int quantityW[], int cardindex, int quantforMen[][30], int quantforWomen[][30]);
...
int checkReviews(string reviews[], int cusCount, string customer[], int reviewindex, int cardindex);
...
int showreviews(string reviews[], int cusCount, string username[], int &counter, int cardindex);
...
void printBillForMen(int menq, int totalM[], int quantityM[], int priceM[], int &sumM, string arrM[], int cardindex, int quantforMen[][30]);
void printBillForWomen(int womenq, int totalW[], int quantityW[], int priceW[], int &sumW, string arrW[], int cardindex, int quantforWomen[][30]);
string billRem1(bool deliveryop, string delivery[], int cardindex);
string billRem1(bool delArea, string delivery[], int cardindex);
int findTotalForBill(int menq, int womenq, int totalM[], int quantityM[], int priceM[], int totalW[], int quantityW[], int priceW[], int cardindex, int quantforMen[][30], int quantforWomen[][30]);
int takeChoiceForBill();
string reasonsForBill(string delivery[], string userArea[], int cardindex);
string billPaid(int menq, int availableM[], int quantityM[], int womenq, int availableW[], int quantityW[], bool &billpaid, int cardindex, int quantforMen[][30], int quantforWomen[][30]);
int takeDelArea();
...
char paymentMethodSelection[range];
char deliveryAreaSelection[range];
    ...
    if(op==1)
    {
        ...
        if(role1=="Employee")
        {
            ...
            else if(employeechoice==4)
            {
                ...
                int go=checkReviews(reviews, cusCount, customer, reviewindex, cardindex);
                if(go!=0)
                    {
                        restrictGo(go);
                    }
            ...
            while(true)
            {
                ...
                if(customerchoice==1)
                {
                    ...
                    if(gender==1)
                    {
                        ...
                        cout << endl;
                        int go=printMitems(arrM, priceM, availableM, quantityM, menq, cardindex, quantforMen);
                        if(go!=0)
                        {
                            restrictGo(go);
                        }
                    ...
                    else if(gender==2)
                    {
                        ...
                        int go=printWitems(arrW, priceW, availableW, quantityW, womenq, cardindex, quantforWomen);
                        if(go!=0)
                        {
                            restrictGo(go);
                        }
                ...
                else if(customerchoice==5)
                {
                    ...
                    int go=bill(arrM, arrW, totalM, totalW, priceM, priceW, quantityM, quantityW, menq, womenq, cardindex, finalTotal, delivery, userArea, deliveryop, delArea, quantforMen, quantforWomen);
                    if(go!=0)
                    {
                        restrictGo(go);
                    }
                ...
                else if(customerchoice==7)
                {
                    ...
                    int go=PayBill(totalM, totalW, quantityM, quantityW, priceM, priceW, availableM, availableW, menq, womenq, delArea, deliveryop, billpaid, cardindex, quantforMen, quantforWomen, delivery, userArea);
                    if(go!=0)
                    {
                        restrictGo(go);
                    }
                ...
                else if(customerchoice==2)
                {
                    ...
                    int go=cart(arrM, arrW, quantityM, quantityW, menq, womenq, cardindex, quantforMen, quantforWomen);
                    if(go!=0)
                    {
                        restrictGo(go);
                    }
                ...
                else if(customerchoice==6)
                {
                    ...
                    int go=updatequantity(quantityM, quantityW, arrM, arrW, menq, womenq, availableM, availableW, counter, cardindex, quantforMen, quantforWomen);
                    if(go!=0)
                    {
                        restrictGo(go);
                    }
...
int bill(string arrM[], string arrW[], int totalM[], int totalW[], int priceM[], int priceW[], int quantityM[], int quantityW[], int menq, int womenq, int cardindex, int finalTotal[], string delivery[], string userArea[], bool deliveryop, bool delArea, int quantforMen[][30], int quantforWomen[][30])
{
    int sumM=0;
    int sumW=0;
    printBillForMen(menq, totalM, quantityM, priceM, sumM, arrM, cardindex, quantforMen);
    printBillForWomen(womenq,totalW, quantityW, priceW, sumW, arrW, cardindex, quantforWomen);
...
string billPaid(int menq, int availableM[], int quantityM[], int womenq, int availableW[], int quantityW[], bool &billpaid, int cardindex, int quantforMen[][30], int quantforWomen[][30])
{
    ...
    billpaid=true;
    resetCart(menq, womenq, quantityM, quantityW, cardindex, quantforMen, quantforWomen); 
...
{
    string reason;
    if(delivery[cardindex]!="" && userArea[cardindex]=="")
    {
        reason="Your delivery Area is not selected yet. You can't Pay Bill.";
    }

    else if(delivery[cardindex]=="" && userArea[cardindex]!="")
    {
    ...
    {
        reason="Your total is 0. So you can't pay the bill.";
    }
...
int PayBill(int totalM[], int totalW[], int quantityM[], int quantityW[], int priceM[], int priceW[], int availableM[], int availableW[], int menq, int womenq, bool &delArea, bool &deliveryop, bool &billpaid, int cardindex, int quantforMen[][30], int quantforWomen[][30], string delivery[], string userArea[])
{
    int total=findTotalForBill(menq,womenq, totalM, quantityM, priceM, totalW, quantityW, priceW, cardindex, quantforMen, quantforWomen);
    cout << "Your total Purchase Amount is Rs " << total << endl;
    if((delivery[cardindex]!="" && userArea[cardindex]!="") && total>0)
    {
        int choice=takeChoiceForBill();
        if(choice==1){
            string result=billPaid(menq, availableM, quantityM, womenq, availableW, quantityW, billpaid, cardindex, quantforMen, quantforWomen);
            cout << result << endl;
            int go=returnforAll();
            return go;  
        }
    ...
    {
        string reason=reasonsForBill(delivery, userArea, cardindex);
        cout << reason << endl;
    }
...
int showreviews(string reviews[], int cusCount, string customer[], int &counter, int cardindex)
{
    cusCount=10;
...
int checkReviews(string reviews[], int cusCount, string username[], int reviewindex, int cardindex)
{
    int counter=1;
    if(reviewindex!=0)
    {
        int list=showreviews(reviews, cusCount, username, counter, cardindex);
    }
